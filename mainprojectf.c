...
void insertRecord(FILE *ms, FILE *metaFile, int ID, const char *info) {
    ...
    rewind(metaFile);
    long metaOffset = -1; // To store the metadata position
    while (fread(&fileMeta, sizeof(meta), 1, metaFile)) {
        if (strcmp(fileMeta.fileName, fileName) == 0) {
            metaOffset = ftell(metaFile) - sizeof(meta);
            break;
        }
    }
    ...
    if (fileMeta.orgGlobal == 0) { // Contiguous organization
        if (fileMeta.orgInternal == 1) { // Sorted
            ...
            while (continu && blockNumber <= fileMeta.taille) {
                ...
                // Save the last record for overflow handling
                record overflowRecord = buffer.trec[buffer.nr - 1];

                // Shift records within the block
                int k;
            ...
            if (continu) {
                // Allocate a new block for the overflow record
    ...
    // Update metadata
    fileMeta.nombreRecords++;
    fseek(metaFile, metaOffset, SEEK_SET);
    fwrite(&fileMeta, sizeof(meta), 1, metaFile);

    printf("Record with ID %d successfully inserted into file '%s'.\n", ID, fileName);
}
...
void deleteRecordPhysical(FILE *ms, FILE *metaFile, int recordID, const char *fileName) {
    ...
    while (fread(&fileMeta, sizeof(meta), 1, metaFile)) {
        if (strcmp(fileMeta.fileName, fileName) == 0) {
            metaOffset = ftell(metaFile) - sizeof(meta); // Save the offset for later update
            break;
        }
    }
    ...
    if (!found) {
        printf("Error: Record with ID %d not found in file '%s'.\n", recordID, fileName);
        return;
    }
...
void deleteFile(FILE *ms, FILE *metaFile) {
    ...
    rewind(metaFile);
    int found = 0;
    long metaOffset = -1; // Store the metadata offset
    while (fread(&fileMeta, sizeof(meta), 1, metaFile)) {
        if (strcmp(fileMeta.fileName, fileName) == 0) {
            found = 1;
            metaOffset = ftell(metaFile) - sizeof(meta);
    ...
    int i;
    for ( i = fileMeta.adresse; i < fileMeta.adresse + fileMeta.taille; i++) {
        releaseBlock(i, ms);
    }

    // Create a temporary file to rewrite the metadata
    FILE *tempFile = fopen("MetaDonnees.temp", "wb");
    if (!tempFile) {
        printf("Error: Could not create temporary metadata file.\n");
        return;
    }
    ...
    // Replace the old metadata file with the updated one
    if (remove("MetaDonnees.data") != 0) {
        printf("Error: Could not remove original metadata file.\n");
        return;
    }

    if (rename("MetaDonnees.temp", "MetaDonnees.data") != 0) {
        printf("Error: Could not rename temporary metadata file.\n");
        return;
    }

    // Reopen the metadata file for further operations
    metaFile = fopen("MetaDonnees.data", "rb+");
    if (!metaFile) {
        printf("Error: Could not reopen metadata file.\n");
        return;
    }
...
void clearms(FILE*ms,FILE *metaFile){
	   	block emptyb={ .nr = 0};
		   ...
		   fseek(ms,0,SEEK_SET);
		    ...
		    clearMetadataFile(metaFile);
    metaFile = fopen("MetaDonnees.data", "rb+"); // Reopen the file for further use
    if (!metaFile) {
        printf("Error reopening metadata file.\n");
    }
...
void clearMetadataFile(FILE *metaFile) {
    ...
    fclose(metaFile); // Close the file before truncating
    metaFile = fopen("MetaDonnees.data", "wb"); // Open the file in write mode to truncate its contents

    if (metaFile) {
        printf("Metadata file cleared successfully.\n");
        fclose(metaFile); // Close the file again after clearing
    } else {
        printf("Error clearing metadata file.\n");
    }
}
